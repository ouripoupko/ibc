...
class ConsensusNavigator:
    ...
    def __init__(self, identity, hash_code, redis_port, mongo_port):
        ...
        contract, self.partners = self.read_from_storage(mongo_port, hash_code)
        self.contract = ContractDialog(self.identity, hash_code, contract, self.partners, redis_port)
    ...
    def a2a_connect(self, record, direct):
        if self.contract.exists() and self.partners and record['message']['msg']['pid'] in self.partners:
            self.logger.warning('%s ~ %-20s ~ %s', record['hash_code'][0:10], 'double a2a_connect', self.identity)
        else:
            self.process(record, direct)
